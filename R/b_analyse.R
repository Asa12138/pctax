...
plot.dist<-function(x,group_df=NULL,...){
  ...
  if(!is.null(group_df)){
    ...
    pheatmap::pheatmap(ab,annotation_row = ann,annotation_col = ann,cluster_rows = FALSE,
...
plot.b_dist<-function(x,mode=1,c_group="inter",...){
  ...
  if(mode==2){
    ...
    p=pl+annotate('text',x = 0.9*max(plot$data[[1]]$x),y = max(plot$data[[1]]$y),
...
b_analyse.data.frame<-function(otutab,norm=TRUE,method=c("pca","nmds"),group=NULL,dist = 'bray',ndim=2,...){
  ...
  if("ca"%in%method){
    ...
    dat.coa$c1[,1:2]%>%dplyr::transmute(contri=((CS1^2/(dat.coa$eig)[1])+(CS2^2/(dat.coa$eig)[2]))*100/2)%>%
      cbind(var_contri,.)->var_contri
  ...
  if("tsne"%in%method){
    ...
    Rtsne::Rtsne(dat.h,dims = ndim,pca = TRUE,
                 max_iter = 1000,
                 theta = 0.4,
    ...
    if(bi==TRUE){
      ...
      if((b_res$var_site%>%dplyr::select(dplyr::starts_with(i))%>%ncol())>0){
        ...
        plist[[i]] <- plist[[i]]+
          ...
          ggrepel::geom_text_repel(data = bi_tmp1, aes(x =  x1*1.05, y = x2*1.05,
                                        color=contri, label = var), size = bi_text_size)+
...
grap_p_test<-function(otutab,metadata,group='Group',nperm = 999,...){
  ...
  ps.expo = phyloseq::phyloseq(phyloseq::otu_table(otutab, taxa_are_rows =TRUE),
...
gp_dis_density<-function(otutab,group){
  vegan::vegdist(t(otutab),method = "bray")%>%
    as.b_dist(group_df = group)%>%plot.b_dist(.,mode=2)
}
...
RDA_plot<-function(phy.rda,Group,metadata=NULL,Group2=NULL,
  ...
  p<-p+labs(x = paste(approach[1],': ', round(100 * rda_eig[1], 2), '%'),
    ...
    geom_segment(data = plotdat[[2]], mapping = aes(x = 0, y = 0, xend = RDA1,yend = RDA2),
                 arrow = arrow(length = unit(0.2, 'cm')), size = 0.5, color = 'blue') +
    do.call(geom_text,pcutils::update_param(list(data = plotdat[[2]], mapping = aes(RDA1 * 1.1, RDA2 * 1.1, label = sample),
                                        color = 'blue', size = 4),env_text_param))
  ...
  if(margin){
    ...
    p=p%>%aplot::insert_right(p1,width = 0.2)%>%aplot::insert_top(p2,height = 0.2)
  }
...
m_group_env<-function(g_otutab,env){
  ...
  all%>%dplyr::mutate(rd = cut(r, breaks = c(-Inf, 0.2, 0.4, Inf),
