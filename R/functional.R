...
COG_desc <- data.frame(
  Category = c("INFORMATION STORAGE AND PROCESSING", "INFORMATION STORAGE AND PROCESSING", "INFORMATION STORAGE AND PROCESSING", "INFORMATION STORAGE AND PROCESSING", "INFORMATION STORAGE AND PROCESSING",
  ...
  Description = c("Translation, ribosomal structure and biogenesis", "RNA processing and modification", "Transcription", "Replication, recombination and repair", "Chromatin structure and dynamics",
...
update_ec_info=function(){
  ...
  ec_node=readxl::read_excel("~/Desktop/element-cycling.xlsx",sheet = 2)%>%as.data.frame()
  ec_link=readxl::read_excel("~/Desktop/element-cycling.xlsx",sheet = 3)%>%as.data.frame()
  ec_gene=readxl::read_excel("~/Desktop/element-cycling.xlsx",sheet = 1)%>%as.data.frame()
  ec_path=readxl::read_excel("~/Desktop/element-cycling.xlsx",sheet = 4)%>%as.data.frame()
  all_ec_info=list(ec_node=ec_node,ec_link=ec_link,ec_gene=ec_gene,ec_path=ec_path)
  save(all_ec_info,file = "~/Documents/R/pctax/data/all_ec_info.rda")
}
...
plot_element_cycle=function(cycle="Nitrogen cycle",anno_df=NULL,only_anno=F,cell_fill=NA,cell_color="orange",
  ...
  if(file.exists("~/Documents/R/pctax/data/all_ec_info.rda"))load("~/Documents/R/pctax/data/all_ec_info.rda",envir = environment())
  else data("all_ec_info",package = "pctax",envir = environment())
  ...
  if(!is.null(anno_df)){
    if(only_anno)ec_gene2=right_join(ec_gene2,anno_df)
    else ec_gene2=left_join(ec_gene2,anno_df)
  }
  ...
  cells=tibble::tribble(
    ...
    "Nitrogen cycle", -2.3,   3.5, -2.3, 2.2,
    "Phosphorus cycle",   -2.7, 2.7, -1.5, 2,
    "Sulfur cycle",   -2.7, 2.7, -2.3, 2.3,
    "Iron cycle",   -2.7, 2.7, -1.2, 1.2
  )%>%as.data.frame()%>%column_to_rownames("Type")
  if(cycle%in%rownames(cells)){
    lib_ps("ggchicklet",library = F)
    p=p+ggchicklet::geom_rrect(data = cells[cycle,],aes(xmin = x1, xmax = x2, ymin = y1, ymax = y2),
  ...
  if(gene_bold&gene_italic)ec_gene2$Gene=paste0('bolditalic(',ec_gene2$Gene,')')
  else if(gene_italic)ec_gene2$Gene=paste0('italic(',ec_gene2$Gene,')')
  else if(gene_bold)ec_gene2$Gene=paste0('bold(',ec_gene2$Gene,')')

  if(!is.null(gene_color))gene_color_param=list(color=gene_color)
  else gene_color_param=list()
  ...
  if(!is.null(anno_df)){
    ...
    if(gene_label){
      p3=p2+do.call(geom_label,pcutils::update_param(list(data = ec_gene2,
                                                 ...
                                                 fill=gene_label_fill,size=gene_size,parse = T,show.legend = F),gene_color_param))
    }
    else{
      p3=p2+do.call(geom_text,pcutils::update_param(list(data = ec_gene2,
                                                 mapping = aes(x=X,y=Y,label=Gene,color = Sub_type),
                                                size=gene_size,parse = T,show.legend = F),gene_color_param))
    }
  }
