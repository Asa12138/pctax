...
#' @param taxlevelchar a vector whose length longer than `ncol(taxdf)`, use to be prefix. Default: c("k", "p", "c", "o", "f", "g","s", "st")
...
#' fillNAtax(taxmat)
fillNAtax<-function (taxdf,taxlevelchar=c("k", "p", "c", "o", "f", "g","s", "st"),
                     na_tax="Unclassified|uncultured|Ambiguous|Unknown|unknown|metagenome|Unassig",ignore.case=TRUE,
                     na_repalce="Unknown") {
  if(length(taxlevelchar)<ncol(taxdf))stop("need taxlevelchar length at least: ",ncol(taxdf))
  if(any(duplicated(taxlevelchar)))stop("taxlevelchar can not be duplicated.")
  ...
  remove_na_taxonomy_rank<-function (x) {
    ...
    if(nchar(na_tax)>0)x[pcutils::grepl.data.frame(na_tax,x, ignore.case = ignore.case)] <- NA
    indx <- vapply(x, function(i) all(is.na(i)), FUN.VALUE = logical(1)) %>%setNames(NULL)
    x <- x[, !indx, drop = FALSE]
    return(x)
  }
  ...
  filltaxname<-function (taxdf, type = "species") {
    ...
    #about 2chars more
    indexmark <- apply(taxdf, 2, \(x) {nchar(x, keepNA = TRUE)}) <= 3
    taxdf[indexmark] <- NA

    if (any(is.na(taxdf[, 1]))) {
      prefix=paste0(taxlevelchar[1],"__")
      taxdf[is.na(taxdf[, 1]), 1] <- paste0(prefix,na_repalce)
    }
    ...
    taxdf <- data.frame(t(mapply(newtaxname, taxdf, indextmp)),stringsAsFactors = FALSE)
  ...
  if (!(grepl(paste0("^",taxlevelchar[1],"__"), taxdf[1, 1]))) {
    tmprownames <- rownames(taxdf)
    tmpcolnames <- colnames(taxdf)
    taxdf <- t(apply(taxdf, 1, as.character))
    taxdf[is.na(taxdf)] <- ""
    taxdf <- data.frame(t(apply(taxdf, 1, \(i)paste(taxlevelchar[seq_len(length(i))], i, sep = "__"))),
...
easy_tree<-function(tree,highlight="Phylum",colorfill="color",pal=NULL,basic_params=NULL,add_abundance=TRUE,add_tiplab=TRUE,fontsize=NULL){
  ...
  else pal=pcutils::get_cols(nrow(phy))
...
get_strip=\(name,tree,flat_n=5){
  ...
  tidytree::offspring(tree,id)%>%dplyr::filter(x==mx)%>%dplyr::arrange(angle)->tmp
...
sangji_plot<-function(tree,top_N=5,notshow=c(),intermediate=FALSE,width = 3000,height = 500,...){
  ...
  tree1%>%dplyr::group_by(level)%>%dplyr::top_n(top_N,abundance)%>%dplyr::ungroup()->sangji_dat
  ...
  #show levels
  sangji_dat%>%dplyr::filter(!label%in%c("","r__root"))%>%dplyr::select(label,level,abundance)%>%as.data.frame()->nodes

  #tree structure
  sangji_dat%>%dplyr::filter(parent_label!="r__root")%>%dplyr::select(label,parent_label,abundance)->links
